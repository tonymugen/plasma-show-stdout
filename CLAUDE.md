...
Current scope: `ScriptOutput` (`plugin/plasma-show-stdout.cpp`) drives an arbitrary, ordered list of modules described by a `std::vector<PSSspace::ModuleSpec>` passed to its constructor (each spec: trigger kind, script path, interval, signal number, output-length limit). The QML-facing constructor currently supplies an **empty** list (via the `defaultSpecs()` helper), so the widget ships with no system-specific scripts baked in — it displays nothing until a config panel feeds it specs. Every module's output is joined with `" | "` into the single `text` property. A `SignalModule` is triggered with `pkill --signal RTMIN+N <host>`, where `<host>` is the process the plugin is loaded into (`plasmashell`, or `plasmawindowed`/`plasmoidviewer` when testing). A config panel for user-defined scripts is the long-term goal but is not yet implemented; populating `defaultSpecs()` with a spec or two is the way to exercise modules until then.
...
- `ScriptOutput` (`QObject`) exposes a `text` `Q_PROPERTY` (`QString`, `NOTIFY textChanged`) to QML. It drives an arbitrary, ordered list of modules described by a `std::vector<PSSspace::ModuleSpec>`. **Two constructors:** the QML entry point `ScriptOutput(QObject *parent)` — which is all `qmlRegisterType` can invoke — delegates to the real `ScriptOutput(std::vector<ModuleSpec>, QObject *parent)`, passing `defaultSpecs()` (a file-scope helper that currently returns an empty vector — the placeholder seam for future config-panel data). Both are `explicit`. `ModuleSpec` is a namespace-scope struct in `plasma-show-stdout.hpp` (so callers/tests can name it): a `Kind` enum (`Timed`/`Signal`), `script` path, `interval` (timed), `signalNumber` (signal), and per-module `outputLimit` (the output-length cap, formerly a single hardwired constant — now set per spec so the config panel can vary it). Each spec is built into a `std::variant<TimedModule, SignalModule>` and a `ModuleSlot`. Modules are run polymorphically via `std::visit([](auto &m){ m(); }, mod)` inside their worker thread (both types expose `void operator()() const`, so **`scriptModules.hpp` needs no base class**); the variant lives only in the spawn path, not in the slot.
